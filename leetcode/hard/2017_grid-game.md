...
- Medium
- [Submission](https://leetcode.com/problems/grid-game/submissions/1515754759/)
...
public:
    long long gridGame(vector<vector<int>>& grid) {
        ...
        for (int i = 0; i < n; ++i) {
            p1[i + 1] = p1[i] + grid[0][i];
            p2[i + 1] = p2[i] + grid[1][i];
        }

        long long res = LONG_LONG_MAX;

        for (int i = 0; i < n; ++i) {
            long long topSum = p1[n] - p1[i + 1];
            long long botSum = p2[i];
            res = min(res, max(topSum, botSum));
        }
        return res;
    }
};
...
- Don't need to calculate the points collected by robot 1, robot 2 can only take the upper right half or the bottom left half from any row change robot 1 performs.

- Robot 1's task is to minimize Robot 2's points, and so, greedy will not work, as both play optimally, Robot 2 can rest assured that Robot 1 will take the best route to give Robot 2 the least number of points, hence, Robot 2 has to only worry about the points it can collect from the top right half or the bottom left half for all the row changes Robot 1 performs.
