...
- Medium
- [Submission](https://leetcode.com/problems/subdomain-visit-count/submissions/1004611311/)
...
public:
    vector<string> subdomainVisits(vector<string>& cpdomains) {
        ...
        for (string cpdomain: cpdomains) {
            int len = cpdomain.find(" ");
            int count = stoi(cpdomain.substr(0, len));
            
            cpdomain = cpdomain.substr(len + 1, cpdomain.size() - 1);
            h[cpdomain] += count;
...
- A `ratio` problem, so nothing really to learn from this.
- Just split the domain names and stored them in a hash table and update the count.
