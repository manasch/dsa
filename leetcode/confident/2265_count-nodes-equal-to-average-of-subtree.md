...
- Medium
- [Submission](https://leetcode.com/problems/count-nodes-equal-to-average-of-subtree/submissions/1089625328/)
...
public:
    int averageOfSubtree(TreeNode* root) {
        ...
        auto dfs = [&] (auto self, TreeNode *node) -> pair<int, int> {
            ...
            int runningSum = leftSum + rightSum + node->val;

            int avg = (runningSum) / (count);
            if (node->val == avg) {
                ++res;
            }
...
- At each node, we need the count of nodes in the subtree and the sum of all the nodes, perform dfs and return the sum and count for each node back to the parent.
