...
- Medium
- [Submission](https://leetcode.com/problems/eliminate-maximum-number-of-monsters/submissions/1093280483/)
...
public:
    int eliminateMaximum(vector<int>& dist, vector<int>& speed) {
        ...
        for (int i = 0; i < n; ++i) {
            times.push_back(ceil((float) dist[i] / speed[i]));
        }
...
- Instead of thinking about in terms of distance, think about it in terms of time.
- Find the time it takes for each of the monster to reach the city, and sort it in ascending order.
- For each minute, check if the nearest monster in terms of time has reached the city or not, if not, increament the time and go to the next monster.
- Can even use a heap to solve the same.
