...
- Medium
- [Submission](https://leetcode.com/problems/evaluate-division/submissions/1127357709)
...
public:
    vector<double> calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries) {
        ...
        for (int i = 0; i < values.size(); ++i) {
            ...
            adj[dst].push_back({src, 1.0 / values[i]});
        }

        auto bfs = [&] (vector<string>& query) -> double {
            ...
            while (!q.empty()) {
                ...
                if (p.first == dst && adj.find(src) != adj.end() && adj.find(dst) != adj.end()) {
                    res = p.second;
                    break;
                }
...
- This is just a bfs / dfs on a weighted graph.
- Convert the inputs to a graph and perform bfs on the src to the dst and keep multiplying the values.
- I do not understand how to use union find on this though.
