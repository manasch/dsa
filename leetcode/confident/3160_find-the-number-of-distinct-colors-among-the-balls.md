...
- Medium
- [Submission](https://leetcode.com/problems/find-the-number-of-distinct-colors-among-the-balls/submissions/1535013529/)
...
public:
    vector<int> queryResults(int limit, vector<vector<int>>& queries) {
        ...
        int n = queries.size();
        vector<int> res(n);
        unordered_map<int, int> colorCnt, ballColor;
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (ballColor[x] != 0) {
                if (--colorCnt[ballColor[x]] == 0) {
                    --cnt;
                }
            }
            ballColor[x] = y;
            if (++colorCnt[ballColor[x]] == 1) {
                ++cnt;
            }
            res[i] = cnt;
        }
...
- tried with arrays, MLE'd - (the number of colors will be way lesser than the limit, hence declaring an array of 1e9 + 1 (which will result in MLE) is useless as it will be sparse, and so using a map is more space efficient.

pigeonhole principle)

- use hashmaps to keep count of color and count and update res.
